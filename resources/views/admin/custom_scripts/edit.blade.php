            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header border-bottom">
                        ...
                        <div>
                            <h5 class="mb-0">Edit Script</h5>
                        </div> 
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('custom-script.update', $script->id) }}">
                            @csrf 
                            @method('PUT')
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-12 col-md-6 mb-3">
                                   ...
                                   <select type="is_active" name="is_active" class="form-select @error('is_active') is-invalid @enderror" required> 
                                       ...
                                       <option value="1" {{ $script->is_active == '1' ? 'selected' : '' }}>Active</option>
                                       <option value="0" {{ $script->is_active == '0' ? 'selected' : '' }}>Non-Active</option> 
                                   </select>
                                   @error('is_active')
                                       <div class="invalid-feedback">{{ $message }}</div>
                                   @enderror
                               ...
                               <div class="col-12 col-md-12 mb-3">
                                   ...
                                   <textarea rows="20" id="script" name="script" style="font-family:monospace;background:black" class="fs-8 form-control text-white @error('script') is-invalid @enderror" value="" required>{{ $script->script }}</textarea>
                                    @error('script')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                               </div> 
                            ...
                            <div class="text-end mt-5">
                                <a href="{{ route('custom-script.index') }}" class="btn btn-secondary">Batal</a>
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire({
                    ...
                    window.location.href = "{{ route('custom-script.index') }}";
                });
            },
            error: function(xhr) {
