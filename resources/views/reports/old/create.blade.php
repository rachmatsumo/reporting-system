                            ...
                            <ul class="nav nav-tabs" id="reportTabs" role="tablist">
                                ...
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="sub-{{ $subDesign->id }}-tab" data-bs-toggle="tab" 
                                        ...
                                        <span class="badge bg-secondary ms-1" id="count-{{ $subDesign->id }}">0</span>
                                    </button>
                                </li>
                                @endforeach
                            </ul>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 mb-3">
                                                    ...
                                                    @include('reports.partials.field-input', [
                                                        ...
                                                        'value' => old("main_data.{$field->name}", $field->default_value)
                                                    ])
...
function addAttendanceRow(fieldSlug, fieldName) {
        ...
        <tr id="attendanceRow-${fieldSlug}-${rowIndex}">
            ...
            <td>
                <button type="button" class="btn btn-danger btn-sm" 
                        onclick="removeAttendanceRow('${fieldSlug}', '${fieldName}', ${rowIndex})">
    ...
    updateAttendanceSummary(fieldSlug, fieldName);
}

function removeAttendanceRow(fieldSlug, fieldName, rowIndex) {
    const row = document.getElementById(`attendanceRow-${fieldSlug}-${rowIndex}`);
    if (row) row.remove();
    reorderAttendanceRows(fieldSlug, fieldName);
}

function reorderAttendanceRows(fieldSlug, fieldName) {
    const tbody = document.getElementById(`attendanceTableBody-${fieldSlug}`);
...
function updateAttendanceSummary(fieldSlug, fieldName) {
    const tbody = document.getElementById(`attendanceTableBody-${fieldSlug}`);
    if (!tbody) return;
    ...
    rows.forEach((row) => {
        ...
        if (userSelect && statusSelect) {
            ...
            if (statusSelect.value && summary.hasOwnProperty(statusSelect.value)) {
    ...
    if (hiddenInput) {
        hiddenInput.value = JSON.stringify(summary);
    }
    console.log(fieldName, summary);
}
...
function createFieldInput(field, name, isTableCell = false) {
    ...
    switch (field.type) {
        case 'attendance':
            const slug = name.replace(/\[|\]/g, '_'); // konsisten pakai _
            return `
                <div class="attendance-container">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <button type="button" class="btn btn-primary btn-sm" onclick="addAttendanceRow('${slug}', '${name}')">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody id="attendanceTableBody-${slug}">
...
@foreach($reportDesign->subDesigns as $subDesign)
    subReportCounters[{{ $subDesign->id }}] = 0;
@endforeach
